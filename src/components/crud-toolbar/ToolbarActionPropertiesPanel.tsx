...
export function ToolbarActionPropertiesPanel({
  ...
  useEffect(() => {
    if (!open) return;
    if (addMode) {
      setLabel(""); setIcon("star"); setVariant("default");
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 16, padding: "16px 20px" }}>
        ...
        <Field label="Icon">
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <Input value={icon} onChange={setIcon} placeholder="download" style={{ flex: 1 }} />
            {icon && (
              <div style={{ padding: "6px 8px", borderRadius: 6, border: "1px solid var(--border)", background: "var(--bg-secondary)" }}>

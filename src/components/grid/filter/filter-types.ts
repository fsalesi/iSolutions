// Filter types and helpers — ported from v1, no external dependencies
...
export const NUMBER_OPS: OpDef[] = [
  ...
  { value: "gte",          label: "Greater or Equal", inputs: 1 },
  { value: "lt",           label: "Less Than",        inputs: 1 },
  { value: "lte",          label: "Less or Equal",    inputs: 1 },
...
export const DATE_OPS: OpDef[] = [
  { value: "eq",           label: "On",           inputs: 1 },
  { value: "neq",          label: "Not On",        inputs: 1 },
  { value: "gt",           label: "After",         inputs: 1 },
  { value: "gte",          label: "On or After",   inputs: 1 },
  { value: "lt",           label: "Before",        inputs: 1 },
  { value: "lte",          label: "On or Before",  inputs: 1 },
  { value: "between",      label: "Between",       inputs: 2 },
  { value: "is_empty",     label: "Is Empty",      inputs: 0 },
  { value: "is_not_empty", label: "Is Not Empty",  inputs: 0 },
];
...
export function findOp(op: FilterOperator): OpDef | undefined {
  for (const l of [TEXT_OPS, NUMBER_OPS, DATE_OPS, BOOL_OPS]) {
    const f = l.find(o => o.value === op); if (f) return f;
  }
  return undefined;
}
...
export function countConditions(node: FilterNode | null): number {
  ...
  if (node.type === "condition") {
    const op = findOp(node.operator); if (!op) return 0;
...
export function cleanTree(node: FilterNode): FilterNode | null {
  if (node.type === "condition") {
    const op = findOp(node.operator); if (!op) return null;
...
export const SEL: React.CSSProperties = {
  ...
  color: "var(--text-primary)", fontSize: 13, padding: "5px 8px",
